...
shiny::conditionalPanel(
  ...
  shiny::tagList(
    ...
    shiny::numericInput("cpsat_workers",   "Parallel workers (Defaults to Number of Dectected Cores - 2)",          value = max(1, parallel::detectCores()-2), min = 1, step = 1),
...
options for designating ASUs. **ASUbuildR** only uses the addition
...
6.5%. The methodology in **ASUbuildR** looks at each census tract with an
...
The basic functionality of **ASUbuildR** is simple. First, install the
package. Second, call the `launch_ASUbuildR()` function. If you prefer,
you don’t even have to attach the package, just call it directly with
`ASUbuildR::launch_ASUbuildR()`. By default, this will launch the
**ASUbuildR** Shiny app in your default browser window.
