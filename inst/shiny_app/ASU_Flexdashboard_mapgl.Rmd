...
# Prevent reactivity issues with empty initial values
map_update_debounced <- shiny::debounce(shiny::reactive({
  list(full_data(), selected_tracts(), highlighted_tracts())
}), 250) # 250ms delay
...
prep_map_data <- function(map_data) {
  ...
  map_data |>
    sf::st_make_valid() |>
    sf::st_simplify(dTolerance = .001, preserveTopology = TRUE) |>  # Simplify geometry
...
# is invalidated on every edit too. If output$initial_map / output$edit_map
...
update_map_efficiently <- function(map_id = "edit_map") {
  data <- map_geom()
  ...
  if (map_id == "edit_map") {
    # Use set_source to update each layer's data in-place (no clear → no blink),
    # then refresh paint and filters. MapLibre calls getSource(id).setData()
    # atomically, so feature-property tooltips also read fresh values.
    sel <- selected_tracts()   %||% character(0)
    hl  <- highlighted_tracts() %||% character(0)

    build_filter <- function(ids) {
      ids <- ids[!is.na(ids)]
      if (length(ids) == 0) return(list("==", c("get", "GEOID"), "__NONE__"))
      if (length(ids) == 1) list("==", c("get", "GEOID"), as.character(ids))
      else list("in", c("get", "GEOID"), list("literal", as.character(ids)))
    }

    mapgl::maplibre_proxy(map_id) |>
      mapgl::set_source(layer_id = "basemap", source = data) |>
      ...
      mapgl::set_filter("selected", build_filter(sel)) |>
      mapgl::set_filter("highlighted", build_filter(hl))
  } else {
    # For read-only maps (initial_map), update source data AND paint property.
    # set_source keeps tooltip feature properties (asunum, rates, etc.) current.
    mapgl::maplibre_proxy(map_id) |>
      mapgl::set_source(layer_id = "basemap", source = data) |>
      mapgl::set_paint_property(
        "basemap", "fill-color",
...
output$cpsat_log_saved_banner <- shiny::renderUI({
  lf <- cpsat_log_saved()
  if (is.null(lf)) return(NULL)
  shiny::div(
    style = "position:fixed; bottom:20px; right:20px; z-index:1100; background:#f0f8ff; border:1px solid #4a90d9; border-radius:4px; padding:10px 14px; font-size:13px; box-shadow:0 2px 8px rgba(0,0,0,.2); max-width:520px;",
    shiny::div(
      style = "display:flex; justify-content:space-between; align-items:flex-start;",
      shiny::strong("Solver log saved:"),
      shiny::actionButton("dismiss_cpsat_banner", "\u00d7",
        class = "btn-xs btn-link",
        style = "padding:0 4px; font-size:16px; line-height:1; color:#555; margin-left:8px;")
    ),
    shiny::span(style = "font-family:monospace; font-size:11px; word-break:break-all;", lf),
    shiny::br(), shiny::br(),
    shiny::actionButton("open_log_folder", "Open folder", class = "btn-xs btn-info", style = "margin-right:6px;"),
    shiny::actionButton("open_log_file",   "Open log",    class = "btn-xs btn-default")
  )
})

shiny::observeEvent(input$dismiss_cpsat_banner, {
  cpsat_log_saved(NULL)
})
...
shiny::observe({
  cpsat_log_buf()  # Depend on this reactive value
  # Instead of session$flushReact(), use invalidateLater
  shiny::invalidateLater(100)  # Check for updates every 100ms
})

```

```{r}
# -- GLOBAL map render that reacts to geoid_set() only --
output$initial_map <- mapgl::renderMaplibre({
  shiny::req(length(geoid_set()) > 0)
  # isolate() so the render body doesn't create additional reactive dependencies
  # that would cause re-sends on asunum-only changes.
  ...
  # Replace 0 with NA so base color applies
...
# Observer for debounced updates — only update edit_map here to avoid
# back-to-back set_source calls that trigger the WebGL READ-buffer warning.
# initial_map is updated explicitly in the CP-SAT / th_pass / th_combine blocks.
...
# The map
  ...
  shiny::uiOutput("cpsat_log_saved_banner")
...
shiny::observeEvent(input$edit_map_feature_click, {
  ...
  # Build filter expression and apply
  if (length(current_selection) > 0 && !any(is.na(current_selection))) {
    filter_expr <- if (length(current_selection) == 1) {
      list("==", c("get", "GEOID"), as.character(current_selection[1]))
    } else {
      list("in", c("get", "GEOID"), list("literal", as.character(current_selection)))
    }
    mapgl::maplibre_proxy("edit_map") |> mapgl::set_filter("selected", filter_expr)
  } else {
    mapgl::maplibre_proxy("edit_map") |> mapgl::set_filter("selected", list("==", c("get", "GEOID"), "__NONE__"))
  }
...
shiny::observeEvent(input$select_asunum, {
  ...
  filter_expr <- if (length(selected_tract_ids) == 1) {
    list("==", c("get", "GEOID"), as.character(selected_tract_ids[1]))
  } else {
    list("in", c("get", "GEOID"), list("literal", as.character(selected_tract_ids)))
  }
...
shiny::observe({
  ...
  if (length(filtered_selected) > 0 && !any(is.na(filtered_selected))) {
    filter_expr <- if (length(filtered_selected) == 1) {
      list("==", c("get", "GEOID"), as.character(filtered_selected[1]))
    } else {
      list("in", c("get", "GEOID"), list("literal", as.character(filtered_selected)))
    }
    mapgl::maplibre_proxy("edit_map") |> mapgl::set_filter("highlighted", filter_expr)
  } else {
...
shiny::observeEvent(input$update, {
  ...
  selected_tracts(NULL)
  # Clear selection with set_filter
  mapgl::maplibre_proxy("edit_map") |>
    mapgl::set_filter("selected", list("==", c("get", "GEOID"), "__NONE__"))
})
...
shiny::conditionalPanel(
  ...
  shiny::div(
    style = "flex: 1; overflow-y: auto; border: 1px solid #dee2e6; border-radius: 5px; min-height: 200px;",
...
shiny::observeEvent(input$summary_table_rows_selected, ignoreNULL = FALSE, {
  ...
  filter_expr <- if (length(geoids) == 0) {
    list("==", c("get", "GEOID"), "__NONE__")
  } else if (length(geoids) == 1) {
    list("==", c("get", "GEOID"), as.character(geoids))
  } else {
    list("in", c("get", "GEOID"), list("literal", as.character(geoids)))
  }
  mapgl::maplibre_proxy("edit_map") |> mapgl::set_filter("highlighted", filter_expr)
})
...
shiny::observeEvent(input$run_asu_summary, {
  ...
  # Contiguity check: for each ASU, build a Queen-contiguity graph over its
  # tracts only and verify the graph is fully connected (one component).
  # Uses the same st_contiguity() approach as the tract-loading step.
  asu_ids <- sort(unique(asu_sf$asunum))
  contiguous_flag <- vapply(asu_ids, function(id) {
    grp <- asu_sf |> dplyr::filter(asunum == id)
    if (nrow(grp) <= 1L) return(TRUE)
    nb <- sfdep::st_contiguity(grp$geometry)
    # sfdep may return 0 for isolated tracts; normalise to integer(0)
    nb <- lapply(nb, function(x) {
