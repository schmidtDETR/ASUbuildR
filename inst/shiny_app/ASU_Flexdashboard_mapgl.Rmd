...
shiny::conditionalPanel(
  ...
  shiny::tagList(
    ...
    shiny::numericInput("cpsat_repair_rounds",   "Repair rounds",              value = 3,    min = 0, step = 1),
    shiny::numericInput("cpsat_repair_timelimit", "Repair time limit (sec)",   value = 60,   min = 0, step = 5),
...
if (identical(algo_choice, "cpsat")) {
  ...
  shiny::withProgress(message = "Running OR-Tools CP-SAT…", value = 0, {
    ...
    repair_rounds <- if (is.null(input$cpsat_repair_rounds))    3L  else as.integer(input$cpsat_repair_rounds)
    repair_tlimit <- if (is.null(input$cpsat_repair_timelimit)) 60  else as.numeric(input$cpsat_repair_timelimit)
    append_cpsat_log(sprintf("[run] params: tau=%.6f pop=%d max=%d limit=%ds workers=%d rel_gap=%s repair_rounds=%d repair_tlimit=%.0fs",
                             tau, pop_thr, max_asus, tlimit, workers,
                             if (is.na(rel_gap)) "None" else as.character(rel_gap), repair_rounds, repair_tlimit))
...
if REL is not None and not (isinstance(REL, float) and math.isnan(REL)):
    kwargs['rel_gap'] = REL
kwargs['repair_enabled'] = %s
kwargs['repair_rounds']  = %d
kwargs['repair_time_limit'] = %s
...
",
      ...
      if (repair_rounds > 0L) "True" else "False", repair_rounds, repair_tlimit,
      out_json
    )
...
output$cpsat_log_saved_banner <- shiny::renderUI({
  ...
  shiny::div(
    ...
    shiny::strong("Solver log saved:"), shiny::br(),
...
# Clear highlights when requested or when ASU selection changes
