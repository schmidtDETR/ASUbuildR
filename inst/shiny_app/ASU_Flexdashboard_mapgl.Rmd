...
source("../../R/run_asu_original.R",  local = TRUE)
source("../../R/run_tract_hunter.R",  local = TRUE)
# source("S:/ra/RADUMP/aisolorio/Code Projects/ASUbuildR/R/run_asu_original.r", local = TRUE)
# source("S:/ra/RADUMP/aisolorio/Code Projects/ASUbuildR/R/run_tract_hunter.r", local = TRUE)
...
>>>>>>> afbd2e3 (Added progress bar, Created relative paths for algorithms. Updated descriptions of algorithms)
...
observeEvent(input$process, {
  ...
  ## ---- 1. geometry housekeeping ---------------------------------
  map_data <- map_data |>
    sf::st_make_valid() |>                 # repair
    sf::st_zm(what = "ZM", drop = TRUE) |> # drop Z/M
    dplyr::filter(!sf::st_is_empty(geometry)) |>     # no empties
  ...
  palette <- if (n_asu == 0)            "#808080"
             else if (n_cat == 1)       "#7FC97F"
             else                       RColorBrewer::brewer.pal(n_cat, "Accent")

  breaks  <- if (n_asu <= 1) numeric(0)
             else seq(1 + n_asu / n_cat, n_asu, length.out = n_cat - 1)

  stops   <- if (length(palette) > 1) palette[-1] else character(0)

# Line below used for debugging geometry types
#print(table(sf::st_geometry_type(map_data)))
...
if (n_asu == 0) {                               # ── 1 · no ASUs yet
  fill_colour <- "#808080"
  legend_vals <- "(none)"
  legend_cols <- "#808080"

} else if (n_asu == 1) {                        # ── 2 · exactly one ASU
  fill_colour <- step_expr(
    column   = "asunum",
    base     = "#808080",                       # grey for NA / 0
    stops    = "#7FC97F",                       # green for ASU 1
    values   = 1,
    na_color = "#444444"
  )
  legend_vals <- 1
...
} else {                                        # ── 3 · two‑plus ASUs
  n_cat   <- min(max(n_asu, 3L), 8L)            # 3–8 categories
...
map |>
  add_fill_layer(
    id                 = "basemap",
    source             = map_data,
    fill_color         = fill_colour,           # <- **now always defined**
    fill_opacity       = 0.5,
