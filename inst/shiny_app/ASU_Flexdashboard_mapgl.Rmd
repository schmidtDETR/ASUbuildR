...
update_map_efficiently <- function(map_id = "edit_map") {
  ...
  # Update colors without recreating layers
...
```{r}
# -- GLOBAL map render that reacts to map_geom() --
output$initial_map <- mapgl::renderMaplibre({
  ...
  # PERFORMANCE: only rebuild the whole map when the tract set changes; ASU
  # edits (same tracts, different `asunum`) are pushed via the existing
  # maplibre_proxy() color updates below instead of a full re-render.
  shiny::bindCache(sort(map_geom()$GEOID))
```
